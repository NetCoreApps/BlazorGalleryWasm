...
public class Migration1002 : MigrationBase
{
    ...
    public override void Up()
    {
        ...
        SeedAttachments(sourceDir,targetDir);
    ...
    public void SeedAttachments(string sourceDir, string targetDir)
    {
        sourceDir.AssertDir();
        ...
        {
            Db.Save(CreatePdfAttachment(targetDir, jobApp, resumeFileInfo));
            Db.Save(CreatePdfAttachment(targetDir, jobApp, coverFileInfo));
        }
    }
    ...
    private static Faker<PhoneScreen> phoneScreenFaker = new Faker<PhoneScreen>()
        .RuleFor(p => p.Id, () => 0)
        .RuleFor(p => p.AppUserId, (f) => ConfigureDbMigrations.allSeedUsers[f.Random.Int(0, ConfigureDbMigrations.allSeedUsers.Count - 1)])
    ...
    private static Faker<JobApplicationEvent> eventFaker = new Faker<JobApplicationEvent>()
        .RuleFor(e => e.Id, () => 0)
        .RuleFor(e => e.AppUserId, (f) => ConfigureDbMigrations.allSeedUsers[f.Random.Int(0, ConfigureDbMigrations.allSeedUsers.Count - 1)])
    ...
    private static Faker<JobApplicationComment> commentFaker = new Faker<JobApplicationComment>()
        .RuleFor(c => c.Id, () => 0)
        .RuleFor(c => c.AppUserId, (f) => ConfigureDbMigrations.allSeedUsers[f.Random.Int(0, ConfigureDbMigrations.allSeedUsers.Count - 1)])
    ...
    public void PopulateJobApplicationEvents(JobApplication jobApp)
    {
        ...
        if (status >= JobApplicationStatus.Offer)
        {
            ...
            appEvent.AppUserId = allIds[FakerInstance.Random.Int(0, allIds.Count - 1)];
            ...
            offer.AppUserId = allIds[FakerInstance.Random.Int(0, allIds.Count - 1)];
            Db.Insert(offer);
        }
        ...
        if (status >= JobApplicationStatus.InterviewCompleted)
        {
            ...
            interview.AppUserId = allIds[FakerInstance.Random.Int(0, allIds.Count - 1)];
            Db.Insert(interview);
        }
        ...
        if (status >= JobApplicationStatus.PhoneScreeningCompleted)
        {
            ...
            screen.AppUserId = allIds[FakerInstance.Random.Int(0, allIds.Count - 1)];
            Db.Insert(screen);
        }
